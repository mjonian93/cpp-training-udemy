...
// Used for test1
// Scoped enumaration holding the items for a grocery list
...
// parameter is valid enumerator or not.
...
void display_grocery_list(const std::vector<Grocery_Item> &grocery_list) {
    ...
    for (Grocery_Item grocery_item: grocery_list){
        std::cout << grocery_item << std::endl;     //Uses the overloaded operator<<
...
// Using a scoped enumaration to model grocery items
void test1() {
    std::cout << "\n---Test1 --------------------------------\n" << std::endl;
...
// Used for test2
// A simple cass that might model a Player in a game application.
...
void test2() {
    ...
    Player p2{"Cloud Strife", Player::Mode::Defense, Player::Direction::West};
    Player p3{"Cloud Strife", Player::Mode::Idle, Player::Direction::South};
