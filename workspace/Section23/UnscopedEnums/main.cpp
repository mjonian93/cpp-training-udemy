...
// Used for test2
// Unscoped enumaration representing items for a grocery shopping list
...
void display_grocery_list(const std::vector<Grocery_Item> &grocery_list) {
    ...
    std::cout << "Invalid itemns: " << invalid_item_count << std::endl;
...
void test3(){
    ...
    switch (state) {
        case Engine_Failure:    // Abort if Engine Failgure
        case Unknown:
            initiate(Abort);
            break;
        case Inclement_Weather:
