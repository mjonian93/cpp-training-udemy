...
void test2() {
    ...
    //Test whether triangle1 and triangle 2 are equivalent
...
//the most common uses of lambdas with the standart template library.
...
//each element for which the predicate returned false-
...
void test5() {
    ...
    //Sort people by name in ascensing order
...
//all_of - Non-modyfing sequence operation
...
public:
    bool is_valid(std::string password) {
        return std::all_of(password.begin(), password.end(), [this] (char character) {
            return std::none_of(restricted_symbols.begin(), restricted_symbols.end(), [character] (char restricted_symbol) {
                return character == restricted_symbol;
            });
        });
    }
...
//a restricted symbol and this is valid or not-
